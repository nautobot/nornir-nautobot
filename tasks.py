...
def run_command(context, exec_cmd, port=None):
    ...
    """
    if is_truthy(context.nornir_nautobot.local):
        print(f"LOCAL - Running command {exec_cmd}")
        result = context.run(exec_cmd, pty=True)
    else:
        print(
            f"DOCKER - Running command: {exec_cmd} container: {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver}"
        )
        if port:
            result = context.run(
...
def build(context, cache=True, force_rm=False, hide=False):
    ...
    print(f"Building image {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver}")
    command = f"docker build --tag {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver} --build-arg PYTHON_VER={context.nornir_nautobot.python_ver} -f Dockerfile ."
    ...
    if result.exited != 0:
        print(
            f"Failed to build image {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver}\nError: {result.stderr}"
        )
...
def clean(context):
    ...
    print(
        f"Attempting to forcefully remove image {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver}"
    )
    context.run(f"docker rmi {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver} --force")
    print(f"Successfully removed image {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver}")
...
def cli(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    dev = f"docker run -it -v {context.nornir_nautobot.pwd}:/local {context.nornir_nautobot.image_name}:{context.nornir_nautobot.image_ver} /bin/bash"
