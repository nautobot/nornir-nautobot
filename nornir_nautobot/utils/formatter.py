"""Command Extraction and Formatting."""
...
def normalize_processed_data(processed_data, iterable_type):  # pylint: disable=too-many-branches
...
def perform_data_extraction(host, command_info_dict, command_outputs_dict, logger, skip_list=None):  # pylint: disable=too-many-locals,too-many-branches
    ...
    for pre_processor_name, field_data in command_info_dict.get("pre_processor", {}).items():
        ...
        # Check for commands and oids key, process them similarly
        commands_key = "commands" if "commands" in field_data else "oids"
        if isinstance(field_data[commands_key], dict):
            # Only one command/oid is specified as a dict, force it to a list.
            loop_commands = [field_data[commands_key]]
        else:
            loop_commands = field_data[commands_key]
