...
class TestTemplateFile(unittest.TestCase):
    ...
    def test_successful_render(self):
        ...
        result = template_file(task=self.task, template="valid.j2", path=self.path, obj=self.obj, logger=logger)
    ...
    def test_undefined_variable(self):
        ...
        with self.assertRaises(UndefinedError):
            template_file(task=self.task, template="undefined_var.j2", path=self.path, obj=self.obj, logger=logger)
    ...
    def test_undefined_attribute(self):
        ...
        self.assertIn("1", log_msg)
        self.assertIn("UndefinedError", log_msg)
        self.assertIn("Color: {{ var1.key2 }}", log_msg)
        self.assertIn("has no attribute 'key2'", log_msg)

    def test_bad_syntax(self):
        """Test rendering with bad syntax."""
        with self.assertRaises(TemplateSyntaxError):
            template_file(task=self.task, template="bad_syntax.j2", path=self.path, obj=self.obj, logger=logger)
        log_records = [record.getMessage() for record in self.buffer_handler.buffer]
        self.assertEqual(len(log_records), 1)  # One error logged
        log_msg = log_records[0]
    ...
    def test_bad_include_file(self):
        ...
        with self.assertRaises(TemplateNotFound):
            template_file(task=self.task, template="bad_include.j2", path=self.path, obj=self.obj, logger=logger)
    ...
    def test_filter_not_found(self):
        ...
        with self.assertRaises(TemplateAssertionError):
            template_file(task=self.task, template="bad_filter.j2", path=self.path, obj=self.obj, logger=logger)
        log_records = [record.getMessage() for record in self.buffer_handler.buffer]
        self.assertEqual(len(log_records), 1)  # One error logged
        log_msg = log_records[0]
    ...
    def test_file_indirection(self):
        """Test rendering with an undefined attribute in an include gives you include file name, not original file."""
        kwargs = {"var1": {"key1": "val1"}}
        with self.assertRaises(UndefinedError):
            template_file(
                task=self.task,
                template="include_with_undefined.j2",
                path=self.path,
                logger=logger,
                **kwargs,
            )
        ...
        self.assertEqual(len(log_records), 1)  # One error logged
        log_msg = log_records[0]
        self.assertIn("Error rendering template `include_with_undefined.j2`", log_msg)
        self.assertIn(os.path.join(self.path, "undefined_var.j2"), log_msg)
        self.assertIn("1", log_msg)
        self.assertIn("UndefinedError", log_msg)
        self.assertIn("Hello {{ name }}", log_msg)
        self.assertIn("Message: `'name' is undefined`", log_msg)
