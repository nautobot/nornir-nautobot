...
ERROR_CODES = {
    ...
    "E1037": ErrorCode(
        ...
        description="Facts crafter failed during get_commands dispatcher execution on `{hostname}`.",
        error_message="Facts crafter failed during get_commands dispatcher execution on `{hostname}`.",
    ...
    "E1038": ErrorCode(
        ...
        description="Facts crafter failed during parse_command_result on `{hostname}`.",
        error_message="Facts crafter failed during parse_command_result on `{hostname}`: `{exception}`.",
        recommendation="Verify that Jinja2 filters are available and correct JPath is used.",
    ),
    "E1039": ErrorCode(
        troubleshooting="Ensure correct filtering and parsing configuration is used.",
        description="Facts crafter failed during data extraction on `{hostname}`.",
        error_message="Facts crafter failed during data extraction on `{hostname}`: `{exception}`.",
        recommendation="Verify that Jinja2 filters are available and correct JPath is used.",
    ),
    "E1040": ErrorCode(
        troubleshooting="Ensure correct schema is used.",
        description="Facts crafter failed during schema validation on `{hostname}`.",
        error_message="Facts crafter failed during schema validation on `{hostname}`: `{exception}`.",
